...
Это указывает на то, что при значительном изменении масштаба используемые признаки перестают различать символы, и классификатор «схлопывается» к одному эталону, вероятно, из-за того, что нормализация признаков не полностью компенсирует изменение размера, а также из-за различий в пропорциях букв.
...
- Использовать дополнительную метрику – нормированную кросс-корреляцию (NCC) по бинарным изображениям.
...
4. **Проведена оценка качества распознавания для базового размера шрифта**  
   ...
   - Выявлены основные причины: несоответствие шрифта и размера эталонных символов, ограниченность набора признаков.
...
- Использовать альтернативные метрики (NCC, профили).  
- Для повышения точности в условиях разного кегля – добавить предобработку (нормализацию размера).
