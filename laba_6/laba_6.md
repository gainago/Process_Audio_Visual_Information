...
Для всех 30 символов алфавита Османья (из папки `../laba_5/osmanya_chars/`) построены эталонные **горизонтальные и вертикальные профили**.
...
### 5. Порядок вызова функций.
Сперва фраза генерируется с помощью generate_message.py затем координаты символов ищутся с помощью find_letters.py. Координаты записываются в segmentation_coords{размер символа}.csv и изоображение с окаймляющими прямоугольниками сохраняется в 
`result/segmented_on_original{размер сивола}.png`. Затем, с помощью segment.py, строятся профиль символов выбранного алфавита.
> На этот раз сегментация символов делается таким же образом как сегментировалис символы алфавита в лабе 5.